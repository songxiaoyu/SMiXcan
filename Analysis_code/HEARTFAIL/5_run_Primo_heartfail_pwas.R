# Run Primo pattern inference on HEARTFAIL PWAS S-MiXcan results.
...
combined_path <- file.path(results_dir, "heartfail_result_pwas.csv")
if (!file.exists(combined_path)) {
  stop("Missing HEARTFAIL PWAS result file: ", combined_path, call. = FALSE)
}
...
out <- primo$out
write.csv(out, file.path(results_dir, "heartfail_result_pwas_annotated.csv"), row.names = FALSE)
...
write.csv(out_rename, file.path(results_dir, "heartfail_table_pwas.csv"), row.names = FALSE)
