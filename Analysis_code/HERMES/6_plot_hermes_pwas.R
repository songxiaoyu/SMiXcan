...
#
# Outputs:
#   Figure/hermes_pwas/HERMES_PWAS_QQ.pdf
#   Figure/hermes_pwas/HERMES_PWAS_QQ_split_venn.pdf
...
figure_dir <- Sys.getenv(
  "HERMES_FIGURE_DIR",
  unset = file.path(paper_dir, "Figure", "hermes_pwas")
)
dir.create(figure_dir, recursive = TRUE, showWarnings = FALSE)

base_font <- 12

plot_prefix <- Sys.getenv("HERMES_PLOT_PREFIX", unset = "HERMES_PWAS")
result_path <- Sys.getenv(
  "HERMES_RESULT_FILE",
  unset = file.path(results_dir, "hermes_result_pwas.csv")
)
annotated_path <- Sys.getenv(
  "HERMES_ANNOTATED_FILE",
  unset = file.path(results_dir, "hermes_result_pwas_annotated.csv")
)
