...
#     Results/hermes_pwas/hermes_workspace/hermes_input/*.rds
#   3_1000Genome_keep_eur_plink_hermes_pwas.sh:
#     Results/hermes_pwas/hermes_workspace/hermes_filtered_id/*.bim/*.raw
#
# HERMES DCM is treated as a binary trait by default.
# Provided counts:
#   cases    = 14256
#   controls = 1199156
# Override with HERMES_N_CASES and HERMES_N_CONTROLS if these counts change.
#
...
family <- Sys.getenv("HERMES_GWAS_FAMILY", unset = "binomial")
if (!family %in% c("binomial", "gaussian")) {
  stop("HERMES_GWAS_FAMILY must be binomial or gaussian.", call. = FALSE)
}

n1 <- as.integer(Sys.getenv("HERMES_N_CASES", unset = "14256"))
n0 <- as.integer(Sys.getenv("HERMES_N_CONTROLS", unset = "1199156"))
if (family == "binomial" && (is.na(n1) || is.na(n0) || n1 <= 0 || n0 <= 0)) {
  stop(
    "Set HERMES_N_CASES and HERMES_N_CONTROLS before running binomial HERMES PWAS.",
    call. = FALSE
  )
}
...
chr_list <- parse_chr_list(Sys.getenv("HERMES_CHR_LIST", unset = paste(1:22, collapse = ",")))
assoc_reg_mode <- Sys.getenv("HERMES_ASSOC_REG_MODE", unset = "fixed")
if (!assoc_reg_mode %in% c("fixed", "estimate")) {
  stop("HERMES_ASSOC_REG_MODE must be fixed or estimate.", call. = FALSE)
}

assoc_reg_scale <- as.numeric(Sys.getenv("HERMES_ASSOC_REG_SCALE", unset = "0.1"))
if (is.na(assoc_reg_scale) || assoc_reg_scale < 0) {
  stop("HERMES_ASSOC_REG_SCALE must be a non-negative number.", call. = FALSE)
}

gene_log_every <- as.integer(Sys.getenv("HERMES_GENE_LOG_EVERY", unset = "50"))
max_genes <- as.integer(Sys.getenv("HERMES_MAX_GENES", unset = "0"))
skip_existing <- parse_bool_env("HERMES_SKIP_EXISTING", default = FALSE)
write_combined <- parse_bool_env("HERMES_WRITE_COMBINED", default = TRUE)

cat("Using HERMES_ASSOC_REG_MODE =", assoc_reg_mode, "\n")
cat("Using HERMES_ASSOC_REG_SCALE =", assoc_reg_scale, "\n")
cat("Using HERMES_SKIP_EXISTING =", skip_existing, "\n")
cat("Using HERMES_WRITE_COMBINED =", write_combined, "\n")
