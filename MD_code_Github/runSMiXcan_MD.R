...
result_total <- vector(mode = "list", length = 9)
t=1
...
for(pheno in pheno_list){
 ...
 # gwas_filtered <- subset(gwas_raw,
 #                       grepl("^\\w$", sapply(strsplit(gwas_raw$MarkerName, ":"), `[`, 3)) &
#                          grepl("^\\w$", sapply(strsplit(gwas_raw$MarkerName, ":"), `[`, 4)))
  ...
  G = length(gene_list)
  result = data.frame(matrix(ncol = 12, nrow = G))
  colnames(result) <- c('gene_id','chr','type','phenotype','celltype','MiXcan_snp_num','filtered_snp_num','Z_1_join','p_1_join','Z_2_join','p_2_join','p_join')
  ...
  for(g in 1:G){
    ...
    gene = gene_list[g]
    print(paste("Processing gene:", gene," in ",cell," for ", pheno))
  ...
  t=t+1
  write.csv(result,paste0(dir_output, pheno,'_',cell,"_SMiXcan_result.csv"))
  }
}

merge_result <- rbindlist(result_total)
write.csv(merge_result,paste0(dir_output,"SMiXcan_result_merged.csv"))
